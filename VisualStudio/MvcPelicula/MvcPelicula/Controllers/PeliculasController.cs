...
namespace MvcPelicula.Controllers
{
    public class PeliculasController : Controller
    {
        ...
        // GET: Peliculas
        public ActionResult Index(string buscarString, string generoPelicula, decimal? precioPelicula=null)
        {
            var PrecioLst = new List<decimal>();
            var PrecioQry = from pr in db.Peliculas select pr.Precio;
            ...
            PrecioLst.AddRange(PrecioQry.Distinct() );
            ViewBag.precioPelicula = new SelectList(PrecioLst);

            var peliculas = from p in db.Peliculas select p;

            if (!String.IsNullOrEmpty(buscarString))
            {
                peliculas = peliculas.Where(s => s.Title.Contains(buscarString));
            }
            if (!string.IsNullOrEmpty(generoPelicula))
            {
                peliculas = peliculas.Where(x => x.Genero == generoPelicula);
            }
            if (!string.IsNullOrEmpty(precioPelicula.ToString()))
            {
                peliculas = peliculas.Where(y => y.Precio == precioPelicula);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,Title,FechaLanzamiento,Genero,Precio")] Pelicula pelicula)
        {
            if (ModelState.IsValid)
            {
